...
{    public string $token;
    ...
    public function store(Request $request, $project_id, $stage_id)
    {
        if(\Auth::user()->can('create project task'))
        {
            ...
            $response =  AsanaTaskController::createTask($request,$project_id);
            $usr        = Auth::user();
            $project    = Project::find($project_id);
            $last_stage = $project->first()->id;
            $post               = $request->all();
            $post['project_id'] = $project->id;
            $post['gid'] = $response['gid'];
...
//        return view('project_task.list', compact('tasks'));
    ...
    public function update(Request $request, $project_id, $task_id)
    {
        ...
        if(\Auth::user()->can('edit project task'))
        {
            ...
            $post = $request->all();
            $task = ProjectTask::find($task_id);
            AsanaTaskController::updateTask($request,$task_id);
            $task->update($post);
    ...
    public function destroy($project_id, $task_id)
    {
        ...
        {
            AsanaTaskController::AsanaDestroy($task_id);
